...
//        String review_date = review.getDateCreated().replaceAll("[a-zA-Z]", " ");
        ...
        setDateAndTime(holder, review_date);
        holder.authore_rating.setRating((float) review.getRating());
        
    }


    private void setDateAndTime(MyViewHolder holder, String date) {
        String [] sereratedDate = date.split("-");

        PersianDate persianDate = new PersianDate();
        int [] jalaliDate = persianDate.toJalali(Integer.parseInt(sereratedDate[0]),Integer.parseInt(sereratedDate[1]),Integer.parseInt(sereratedDate[2]));

        String concatinatedDate = jalaliDate[0] + "/" + jalaliDate[1] + "/" + jalaliDate[2];

        holder.authore_date.setText(Utilities.convertToPersian(concatinatedDate));
    }
